...
def get_arguments():
    ...
    parser.add_argument("-t", "--Threads", dest="threads",
            ...
            type=int,
            default=2)
    parser.add_argument("-py", "--Python3", dest="python3", help="Path and command to run Python3",
            metavar="PATH", required=True)       # MB Jan2020     Windows

    parser.add_argument("-samt", "--Samtools", dest="samtools", help="Path and command to run samtools",
            metavar="PATH", required=True)       # MB Jan2020    Windows
...
def execute_mpileup(header, bam_file, pileupfile, Quality_thresh, folder):
    ...
    cmd = args.samtools + " mpileup -AQ{} -r {} {} > {}".format(Quality_thresh, header, bam_file, pileupfile)  # MB Jan2020
...
def chromosome_table(bam_file,bam_folder,file_name):
    ...
    f = open(tmp_output, "w")
    subprocess.call([args.samtools, "idxstats", bam_file], stdout=f)		# MB Jan2020 Windows
    ...
    f.close()										# NB Jan2020 Bug

    #cmd = "rm "+tmp_output							# MB Jan2020
    #subprocess.call(cmd, shell=True)				# MB Jan2020
...
def get_folder_name(path_file):
    ...
    print("folder " +folder)						# MB Jan2020 debug
    folder_name = os.path.splitext(folder)[0]
    print("folder_name " +folder_name)				# MB Jan2020 debug
    return folder_name
...
def create_tmp_dirs(folder):
    ...
    if os.path.isdir(folder):
        while(flag):
            flag = False							# MB Jan2020 Windows / debug (for batch runs)
            return									# MB Jan2020
            '''print("WARNING! File "+folder+" already exists, \nWould you like to remove it?")
            choice = input("y/n: ")            		# MB Jan2020 Code section commented out start above
            if str(choice) == "y":
                cmd = 'rm -r '+folder
                subprocess.call(cmd, shell=True)
                cmd = 'mkdir '+folder
                ...
                print("Please type y or n")'''      # MB Jan2020 code secetion comment out end
    else:
        # cmd = 'mkdir '+folder						# MB Jan2020 Windows
        # subprocess.call(cmd, shell=True)          # MB Jan2020 Windows
...
def samtools(threads, folder, folder_name, bam_file, Quality_thresh, Markerfile):
    ...
    '''											# MB Jan2020 Our use guarantees BAI exists; start comment out
    if not os.path.exists(bam_file+'.bai'):

        bam_file_order = folder+os_slash+folder_name+".order.bam"
    ...
    '''               							# MB Jan2020 end comment out
    file_name  = folder_name
    Outputfile = folder+os_slash+folder_name+".out"     # MB Jan2020 Windows
    log_output = folder+os_slash+folder_name+".log"		# MB Jan2020 Windows
    fmf_output = folder+os_slash+folder_name+".fmf"		# MB Jan2020 Windows
    pileupfile = folder+os_slash+folder_name+".pu" 		# MB Jan2020 Windows
    ...
    #cmd = "rm {};".format(pileupfile)					# MB Jan2020 Windows
    #subprocess.call(cmd, shell=True)					# MB Jan2020 Windows
...
def identify_haplogroup(app_folder,path_file, output):
    ...
    script = app_folder+os_slash+"predict_haplogroup.py"	# MB Jan2020 Windows
    cmd = args.python3 + " {} -input {} -out {}".format(script, path_file, output) # MB Jan2020 Windows
    print(cmd)
    subprocess.call(cmd, shell=True)
...
if __name__ == "__main__":
    ...
    if os.path.isabs(out_path):
        ...
        if cwd == os_slash:							# MB Jan2020 Windows
            out_folder = out_path
        else:
            out_folder = cwd+os_slash+out_path		# MB Jan2020 Windows
    out_folder = out_folder							# MB Jan2020 on drugs? bug added?

    #print ("App folder: " + app_folder)			# MB Jan2020 debug
    #print ("Out path: " + out_path)				# MB Jan2020 debug
    #print ("Cwd: " + cwd)							# MB Jan2020 debug
    #print ("Out folder: " + out_folder)			# MB Jan2020 debug
        ...
        if args.Fastq:
                ...
                for path_file in files:
                    ...
                    if create_tmp_dirs(folder):
                        start_time = time.time()
                        sam_file = folder+os_slash+folder_name+".sam"       # MB Jan2020 Windows
                        ...
                        start_time = time.time()
                        bam_file = folder+os_slash+folder_name+".bam"        # MB Jan2020 Windows
                        ...
                        #cmd = "rm {}".format(sam_file)						# MB Jan2020 Windows
                        #subprocess.call(cmd, shell=True)           		# MB Jan2020 Windows
                #hg_out = out_folder+os_slash+out_path+".hg"				# MB Jan2020 Windows
                hg_out = out_path+".hg"										# MB Jan2020 ??? bug ; removed batch
