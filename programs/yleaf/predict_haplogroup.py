...
import re
import argparse, os
import platform                         # MB Jan2020
from argparse import ArgumentParser

# Help make platform independent by using backslash for Win10 systems   #REH 11Mar2020

os_slash = "\\" if (platform.system()=="Windows") else "/"     # MB Jan2020  For Windows
...
# If PATH is folder, walk down tree and return all files with EXTension         # REH 11Mar2020
# Otherwise, simply return PATH provided (assuming it is a file)                # REH 11Mar2020
# Needed because provide a batch procesing faeture                              # REH 11Mar2020
...
def get_intermediate_branch(init_hg,path_hg_prediction_tables):
    # Init_hg is the single letter haplogroup identified (major clade)
    # path_hg_prediction_tables is the path to the applications tables of SNPs
    #tmp_init_hg = init_hg+"_int.txt"             # REH 11Mar2020 simplification
    hg_intermediate_file = path_hg_prediction_tables+init_hg+"_int.txt"     # REH 11Mar2020
    try:
...
def get_putative_hg_list(init_hg, hg, df_haplogroup_trimmed, df_haplogroup_all):
    ...
    dict_hg = {}
    list_hg_remove = []
    hg_threshold = 0.7                  # MB Jan2020 relaxed from 0.8 in original
...
def calc_score_three(df_haplogroup,putative_hg):
    ...
    for putative_hg in [putative_hg]:
        ...
        try:
            ...
            qc_three = 0.0
    #print(qc_three)				# MB Jan2020 debug
    return qc_three
...
def process_keys(keys):
    ...
    '''for i in tmp_hg:             # MG Jan2020
        if i in keys:
            index = keys.index(i)
            keys.pop(index)'''      # MG Jan2020
    return keys, tmp_hg

# Open a .log file of passed in base name and return                            # REH 11Mar2020
# the first line (Total Reads) and last (# valid markers with haplogroups)      # REH 11Mar2020
def process_log(log_file):
    log_file += "log"       # arggh, stripped off extension but left dot        # REH 11Mar2020
    total_reads = "NA"
    valid_markers = "NA"

    try:
        df_log = pd.read_csv(log_file, sep=":", header=None)    # Pamda Read .log file  # REH 11Mar2020
        log_array = df_log[1].values        # Second column; in value format            # REH 11Mar2020
        total_reads = log_array[0]          # First line (total reads)                  # REH 11Mar2020
        valid_markers = log_array[-1]       # Last line (markers with haplogroup info)  # REH 11Mar2020
    except FileNotFoundError:
        print("Warning: log file not found!")
    return total_reads, valid_markers

# Start of Main Program (entry); only execute if not imported                   # REH 11Mar2020
if __name__ == "__main__":
    ...
    # Check if input a folder; find all sub-files with .out extension if so     # REH 11Mar2020
    path_samples = args.Input
    samples = check_if_folder(path_samples,'.out')

    # Save output file specification; find needed files in programs subdirs     # REH 11Mar2020
    out_file = args.Outputfile                      # restored to file spec     # REH 11Mar2020
    home_source = os.path.dirname(os.path.realpath(__file__))
    hg_intermediate = home_source+os_slash+"Hg_Prediction_tables"+os_slash      # MG Jan2020 Windows
    intermediate_tree_table = hg_intermediate+"Intermediates.txt"   # File (path above) # REH 11Mar2020

    h_flag = True
    log_output = []
    for sample_name in samples:     # Possible array of samples to process if batch
        print(f"Predicting haplogroup for sample: {sample_name}")
        putative_hg = "NA"
        out_name = sample_name.split("/")[-1]   # Extract file name from path with file  # REH 11Mar2020
        out_name = out_name.split(".")[0]       # Extract base file name (strip extension .out)

        # Arggh: pass sample_name file name after stripping just "out" and not dot "."  # REH 11Mar2020
        total_reads, valid_markers = process_log(sample_name[:-3])

        # Read in pre-stored Intermediates.txt file
        ...
        #print(putative_hg)
        if len(tmp_hg) == 0:                # MB Jan2020
            tmp_hg.append(putative_hg)      # MB Jan2020
        ...
        if putative_hg == "NA":
            ...
            out_hg = putative_hg[0] + "-" + marker_name[0] + ("/etc" if len(marker_name) > 1 else "")
            if len(putative_ancestral_hg) > 0:
                out_hg += "*(x"
        ...
        # Setup for Batch multiple runs; only write header in first instance (as told by flag) # REH 11Mar2020
        ...
        # Also save a less strict haplogroup prediction, as the former          # MB Jan2020
        # one often misses the real terminal haplogroup                         # MB Jan2020
        with open(out_file + ".maxhg", "a") as fp:                              # MB Jan2020
            fp.write(max(tmp_hg, key=len))                                      # MB Jan2020
